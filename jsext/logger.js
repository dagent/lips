...
var FVL_LOG_ID = 'fvlogger-entries';
...
function getTimeStr() {
	...
	strTime = dateCurrent.format(dateFormat.masks.isoTime);
	return strTime;
}
...
function FVL_log(message, level) {
	...
	var strTime = getTimeStr();
	var strOut = "[" + strTime + "]  " + message;
	//var text = document.createTextNode(strOut);
	//p.appendChild(text);
	
	// Changed to this so embedded html will be rendered
	p.innerHTML = strOut + p.innerHTML;
